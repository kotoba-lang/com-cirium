(ns cirium.main
  "Kotodama WASM entrypoint for the Cirium clean-room actor (L4) — Clojure port.
  ...
  facts (`cirium.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cirium")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"cirium.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cirium-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cirium-compat/ready)
